...
int check_fail(uint player, uint huddle, uint status)
{
	...
	{
		int x = huddleInfo[huddle].get_Xpos(), y = BIRD_START_Y;
		for (int i = playerInfo[player].get_Xpos(); i < playerInfo[player].get_Xpos() + 9; i++)
		{
			for (int j = playerInfo[player].get_Ypos(); j < playerInfo[player].get_Ypos() + 11; j++)
			{
...
int Playing()
{
	...
	while (1) {
		...
		finish = time(NULL);
		time(&finish);
		if (((int)difftime(finish, start)) > (huddle_speed - 1) * 10) huddle_speed++;
		...
		//현재 점수 출력
		ShowScore((int)difftime(finish, start));
